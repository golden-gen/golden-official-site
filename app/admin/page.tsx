...
// --- CONFIGURATION ---
const API_URL = 'https://registration-api-qvby.onrender.com/api/v1/reg/students';
const MOCK_USER_EMAIL = ['abdrahmanoladimeji02@gmail.com', 'adebayorfaruqadebayo@yahoo.com', 'goldengenerationoyo@gmail.com'];
...
const Dashboard: React.FC = () => {
    ...
    const fetchStudents = async (): Promise<void> => {
      ...
      try {
        ...
        if (result && Array.isArray(result.data)) {
          ...
          throw new Error('Invalid data format: Expected an array in the "data" property.');
        }
  ...
  const overviewData: OverviewData = useMemo(() => {
    if (students.length === 0) {
      return { total: 0, classes: {}, genders: { 'male': 0, 'female': 0, 'N/A': 0 } };
    }

    // Explicitly type the accumulator object for class counts
    const classCounts: Record<string, number> = {};
    const genderCounts: OverviewData['genders'] = { 'male': 0, 'female': 0, 'N/A': 0 };
    ...
    return (
      <div style={styles.loginContainer}>
        <form onSubmit={handleLogin} style={styles.loginForm}>
          <h2 style={{ marginBottom: '20px' }} className='text-center font-bold text-2xl'>Admin Login</h2>
          ...
          <input
            ...
            required
          />
          <button type="submit" style={styles.loginButton} className='bg-primary'>Log In</button>
        </form>
      </div>
    );
  }
  ...
  if (loading) {
    return <div style={{ padding: '40px', textAlign: 'center' }} className='min-h-[40vh] flex items-center justify-center'>Loading dashboard data...</div>;
  }

  if (error) {
    return <div style={{ padding: '40px', color: 'red', textAlign: 'center' }} className='min-h-[40vh] flex items-center justify-center'>Error: {error}</div>;
  }
      ...
      <div style={styles.cardContainer}>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.cardContentList}>
            ...
            {Object.entries(overviewData.classes).map(([className, count]: [string, number]) => (
        ...
        <div style={{ overflowX: 'auto' }}>
            ...
            <tbody>
                ...
                <tr key={student._id} style={styles.tableRow}>
                    ...
                    <td style={styles.tableCell}>{`${student.surname} ${student.othersName}`}</td>
                    <td style={styles.tableCell}>{student.school || 'N/A'}</td>
                    <td style={styles.tableCell} className='uppercase'>{student.class || 'N/A'}</td>
                    <td style={styles.tableCell} className='capitalize'>{student.gender || 'N/A'}</td>
